    ...
    <div className="space-y-6 max-w-4xl">
      ...
      <div>
        <div className="flex items-center gap-3 mb-4">
          ...
          <div>
            <p className="text-sm font-semibold text-gray-900">AI Ad Copy Generator</p>
            <p className="text-xs text-gray-400">Generate ad copy powered by Cerebras llama-3.3-70b</p>
          </div>
        </div>
        ...
        <Card>
          <div className="grid grid-cols-2 gap-4 mb-4">
            <div>
              ...
              <select
                value={adCopyPlatform}
                onChange={(e) => setAdCopyPlatform(e.target.value)}
                className="w-full px-3 py-2.5 text-sm border border-gray-200 rounded-xl focus:outline-none bg-white text-gray-700"
              >
            ...
            <div>
              ...
              <input
                type="text"
                value={adCopyProduct}
                onChange={(e) => setAdCopyProduct(e.target.value)}
                placeholder="e.g. AI Marketing Dashboard"
            ...
            <div>
              ...
              <input
                type="text"
                value={adCopyAudience}
                onChange={(e) => setAdCopyAudience(e.target.value)}
          ...
          {generatedCopies.length > 0 && (
            <div className="mt-5 space-y-3">
              <p className="text-xs font-semibold text-gray-700 uppercase tracking-wider">Generated Variants</p>
              {generatedCopies.map((copy, i) => (
                <div key={i} className="p-4 bg-gray-50 rounded-xl border border-gray-100">
                  <div className="flex items-start justify-between gap-2 mb-2">
                    <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wider">Variant {i + 1}</span>
                    <Button
                      variant="ghost"
                      size="sm"
                      icon={<Copy className="w-3 h-3" />}
                      onClick={() => navigator.clipboard.writeText(`${copy.headline}\n${copy.description}\n${copy.cta}`).catch(() => {})}
                    >
                      Copy
                    </Button>
                  </div>
                  <p className="text-sm font-semibold text-gray-900 mb-1">{copy.headline}</p>
                  <p className="text-xs text-gray-600 mb-2">{copy.description}</p>
                  <span className="inline-flex items-center px-2.5 py-0.5 rounded-full bg-gray-900 text-white text-[10px] font-medium">
                    {copy.cta}
                  </span>
                </div>
              ))}
